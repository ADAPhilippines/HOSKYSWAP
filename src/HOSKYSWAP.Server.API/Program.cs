...
app.MapGet("/order/{address}", async ([FromRoute] string address, HoskyDbContext dbContext) =>
{
	if (dbContext.Orders is not null)
	{
		return await dbContext.Orders.Where(o => o.OwnerAddress == address && o.Status == Status.Filled || o.Status == Status.Canceled).ToListAsync<Order>();
	}
	else
		throw new Exception("Server error occured. Please try again.");
});
...
app.MapGet("/order/{address}/open", async ([FromRoute] string address, HoskyDbContext dbContext) =>
{
	if (dbContext.Orders is not null)
	{
		return await dbContext.Orders.Where(o => o.OwnerAddress == address && o.Status == Status.Open && o.Status == Status.Cancelled).OrderByDescending(o => o.CreatedAt).ToListAsync<Order>();
	}
	else
		throw new Exception("Server error occured. Please try again.");
});
